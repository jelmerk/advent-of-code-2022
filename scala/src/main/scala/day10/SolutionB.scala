...
object SolutionB extends App {
  ...
  val result = (0 until screenHeight).map { y =>
    (0 until screenWidth).map { x =>
      val cycle = y * screenWidth + x

      val occupies = states
        .takeWhile(_.runtime <= cycle)
        .lastOption
        .map { state => Set(state.x - 1, state.x, state.x + 1) }
        .getOrElse(Set.empty)

      if (occupies.contains(x)) '#'
      else '.'
    }.mkString
  }.mkString(lineSeparator)

  println(result) // EHPZPJGL
