...
object SolutionB extends App {
  ...
  val allDividers = stages
    .map(_.divider)

  val commonDivider = Iterator.from(1)
    .find { candidate =>
      allDividers.forall { divider => candidate % divider == 0 }
    }
    .head
    ...
    stages.foldLeft(roundState) {
      case (State(inventory, inspectionCounts), Stage(id, expression, divider, pass, fail)) =>
        ...
        val newInventory = items.foldLeft(inventory.updated(id, List.empty)) { case (acc, item) =>
          ...
          val boredWorryLevel =
            if (result > commonDivider) result - ((result / commonDivider) * commonDivider)
            else result
