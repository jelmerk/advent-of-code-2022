...
object SolutionB extends App {
  ...
  val tiles = Range.inclusive(minY, maxY + 2).map { y =>
    Range.inclusive(minX - xPadding, maxX + xPadding).map {
      case _ if y == maxY + 2 => Rock
      case x =>
  ...
  @tailrec def update(grid: Grid): Grid = {
    ...
    if (restingCoords == start) newGrid
    else update(newGrid)
  }

  val finalGrid = update(grid)

  println(finalGrid.countSand()) // 23390
